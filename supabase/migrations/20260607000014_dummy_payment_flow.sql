create or replace function mark_dummy_payment_as_paid(target_payment_id uuid)
returns uuid
language plpgsql
security definer
...
begin
  ...
  values (
    ...
    v_user_id,
    'Pembayaran dummy berhasil disimulasikan'
  );
...
revoke execute on function mark_dummy_payment_as_paid(uuid) from public;
grant execute on function mark_dummy_payment_as_paid(uuid) to authenticated;
